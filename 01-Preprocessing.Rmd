...
The entire preprocessing workflow is automated and available as a snakemake file **here**
...
## Count reads
We then use a file containing variable width bins, which can be found **here**, and count the reads in each genomic region. We use `bedtools intersect` to do this. We run this per sample and then combine all the samples (normally all 384 cells from one library) into one large file. 
...
Just like the preprocessing section, the copy number calling is also automatized and wrapped in a snakemake pipeline and is available **here**.
